...
Every section 5.4 P1 feature is now built. Significant figures exist as a
per-sheet setting, and tags were deliberately not added, as the
...
- [x] Service and App Intent for pure expression calculation. Evaluate Expression and Calculate Expression share one bounded evaluation, and Open Sheet, the §5.4 intent about windows rather than answers, shows the sheet a title names; see docs/integration/headless-answers.md.
...
- [x] Scrub a selected numeric literal with Option-drag plus keyboard alternative. Option-dragging sideways over a number steps it by one unit of the place it was written to, with Shift for ten of that place and Command for a tenth, and every answer that depends on it follows; the drag counts from where it began and undoes as one change. Step Number Up ⌃↑ and Step Number Down ⌃↓ do the same from the keyboard. See docs/editor/text-view.md.
...
- Zero known data-loss, crash, silent-wrong-answer or P0 accessibility bugs. Met as far as the evidence reaches: 207 golden answers, the property suites, 38 fuzz seeds plus 1,000 generated inputs per run and 50,000 nightly under Address Sanitizer, `StorageFaultTests`, `RecoveryRehearsalTests`, and the Numi comparison found none. One crash did reach this state and was fixed on 2026-09-16: the shipped app stopped the first time a line failed, because `Bundle.module` cannot find this module's strings inside a sandboxed app. Nothing in the suites could see it, as they all run in the package layout; running the signed app until it had to word a failure is what found it. `ModuleBoundaryTests` now keeps `Bundle.module` out of `Sources`, which is what stops the cause returning, and docs/release/distribution.md asks for a failing line in a signed run before release, which is what would catch another way in. No beta reports exist yet to contradict the criterion.
- All hard performance/footprint budgets pass or have evidence-backed approved ADR revisions. Every budget now has a measured result on the development Mac and all of them pass, with no revision needed; see Benchmarks/Results/phase-11-performance.md. Measured again on 2026-09-16 with every §5.4 P1 feature in the build; all still pass. Two caveats: the 10,000-line chained-edit gate passes at 48.7 ms against 50 ms, 1.3 ms of headroom, and the "no periodic wakeups" half of the idle-CPU gate is unproven because `powermetrics` needs administrator authorization, though idle CPU time is 0.15–0.5% of one core.
