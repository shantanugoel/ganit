...
public struct SheetEvaluation: Hashable, Sendable {
  ...
  /// Lines whose expressions were parsed and evaluated in this generation,
  /// in sheet order. Every other line reused its previous result.
...
public struct SheetCalculator: Sendable {
  ...
  public mutating func evaluate(
    ...
    for line in sheet.lines {
      ...
      var current =
        cache[line.id].flatMap { $0.text == line.text ? $0 : nil }
        ?? CachedLine(line.text, engine: engine, context: context)
      if case .calculation(_, _, let expressionRange?, _) = current.syntax {
        let names = scope.values(named: current.words)
        if !current.canReuseEvaluation(names: names, outcomes: outcomes) {
          current.evaluate(
            expressionRange,
            engine: engine,
            context: context,
      ...
      cache[line.id] = current
      let result = current.evaluation?.result
      if let name = current.declaredName, let result {
      ...
      case .value(let value):
        outcomes.append(.value(value), references: current.evaluation?.references ?? [])
      case .syntaxFailure, .evaluationFailure:
        outcomes.append(.failure, references: current.evaluation?.references ?? [])
      }
      switch current.syntax {
      case .blank, .heading:
        outcomes.endBlock()
      case .divider:
        outcomes.endBlock()
      ...
      results.append(SheetLineResult(id: line.id, syntax: current.syntax, result: result))
    }
...
/// Everything derived from a line's text, plus its last evaluation and the
/// inputs that evaluation read.
private struct CachedLine: Sendable {
  ...
  init(_ text: String, engine: CalculationEngine, context: EvaluationContext) {
    self.text = text
  ...
  func canReuseEvaluation(
    names: [String: EngineValue?],
    outcomes: LineOutcomes
  ) -> Bool {
    guard let evaluation, evaluation.names == names else {
      return false
    }
    return evaluation.inputs.allSatisfy { outcomes.inputs(for: $0.key) == $0.value }
  }

  mutating func evaluate(
    _ expressionRange: SourceRange,
    ...
    if let nameFailure {
      evaluation = Evaluation(result: nameFailure, names: names, references: [], inputs: [:])
      return
    }
    let (result, expression) = engine.evaluate(
      slice(of: expressionRange, in: text),
      context: context,
    ...
    evaluation = Evaluation(
      ...
      inputs: Dictionary(uniqueKeysWithValues: references.map { ($0, outcomes.inputs(for: $0)) })
    )
  }
}

/// Visible variables in declaration order, with an index of name prefixes for
/// multi-word lookup.
private struct VariableScope: Sendable {
  private(set) var variables: [String: EngineValue?] = [:]
  ...
  /// can only resolve names in this set.
